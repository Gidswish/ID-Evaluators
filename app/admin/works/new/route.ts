...
export async function POST(req: NextRequest) {
  ...
  if (!title || !slug) {
    redirectUrl.searchParams.set(
      "error",
      "Missing title or slug when creating evaluation."
    );
    return NextResponse.redirect(redirectUrl);
  }

  let report_url: string | null = null;
  let report_file_path: string | null = null;
  let cover_image_url: string | null = null;
  let cover_image_path: string | null = null;

  // NOTE: We are using the existing bucket "evaluation-files".
  // Reports go under reports/, covers go under covers/.
  ...
  if (error) {
    ...
    redirectUrl.searchParams.set(
      "error",
      "Failed to save evaluation. Check console / Supabase logs."
    );
  }
